...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("name")
        .setDescription(`${language.__n(`staff.staff_name`)}`)
        .setRequired(true)
    ),
    ...
    try {
      ...
      if (!staffData) {
        return interaction.reply(
          `${language.__n(`global.no_results`)} **${staffName}**`
        );
      }

      const embed = new MessageEmbed()
        .setTitle(
          `${language.__n(`staff.staff_info: `)} ${staffData.name.first} ${
            staffData.name.last
          }`
        )
        .setURL(staffData.siteUrl)
        .setDescription(
          staffData.description || `${language.__n(`global.unnavaliable`)}`
        )
