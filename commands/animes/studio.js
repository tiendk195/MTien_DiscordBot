...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("name")
        .setDescription(`${language.__n(`studio.studio_name`)}`)
        .setRequired(true)
    ),
    ...
    try {
      ...
      const animeList = studioData.media.nodes
        .map((anime, index) => {
          ...
          const animeYear = anime.startDate
            ? anime.startDate.year
            : `${language.__n(`global.unavailable`)}`;
          return `${index + 1}. [${animeTitle}](${animeUrl}) - ${language.__n(
            `studio.product_year`
          )}: ${animeYear}`;
        })
        .join("\n");
      ...
      const updateEmbed = () => {
        ...
        const displayedAnime = animeList
          .split("\n")
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            `${language.__n(`studio.product_list`)} ${
              studioData.name
            }:\n${displayedAnime}`
          )
          .setTimestamp();
