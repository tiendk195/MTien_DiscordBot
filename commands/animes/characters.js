...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("name")
        .setDescription(`${language.__n(`character.character_name`)}`)
        .setRequired(true)
    ),
    ...
    try {
      ...
      if (!characterData) {
        return interaction.reply(
          `${language.__n(`global.no_results`)} **${characterName}**`
        );
      }

      let description =
        characterData.description || `${language.__n(`global.no_description`)}`;
      ...
      const embed = new MessageEmbed()
        ...
        .addFields({
          name: `${language.__n(`character.anime_appearances`)}`,
          value:
            characterData.media.nodes
              .map((node) => node.title.romaji)
              .join(", ") || `${language.__n(`global.no_results`)}`,
        })
