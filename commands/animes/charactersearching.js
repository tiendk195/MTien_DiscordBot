  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("character")
        .setDescription(
          `${language.__n(`character_search.command_description`)}`
        )
        .setRequired(true)
    ),
    ...
    try {
      ...
      if (!characterData) {
        return interaction.reply(
          `${language.__n(`global.no_results`)} **${character}**`
        );
      }

      const embed = new MessageEmbed()
        .setTitle(
          `${language.__n(`character_search.anime_list`)} ${
      ...
      characterData.media.nodes.forEach((anime) => {
        const animeTitle =
          anime.title.romaji || `${language.__n(`global.unavailable`)}`;
