  ...
  async execute(interaction) {
    ...
    if (commandCooldown.has(interaction.user.id)) {
      ...
      if (currentTime - lastUsage < cooldownTime) {
        ...
        return interaction.reply(
          `${language.__n(`global.tracemoe_api_limit`)} ${language.__n(
            `global.retry`
          )} ${remainingMinutes} ${language.__n(`global.minute`)}.`
        );
      }
    }
    ...
    try {
      ...
      if (data.result && data.result.length > 0) {
        ...
        const animename =
          response.data.data.Media.title.english ||
          response.data.data.Media.title.romaji ||
          response.data.data.Media.title.native;
        ...
        const embed = new MessageEmbed()
          ...
          .setDescription(
            `${language.__n(`global.description`)}: ${description}`
          )
          .addFields(
            {
              name: `${language.__n(`search.appears_episode`)}`,
              value: `${episode}`,
              inline: true,
            },
            {
              name: `${language.__n(`search.similarity`)}`,
              value: `${similarityINT} %`,
              inline: true,
            }
          )
      ...
      if (error.response && error.response.status === 413) {
        ...
        return interaction.reply(
          `${language.__n(`search.tracemoe_api_limit`)}`
        );
