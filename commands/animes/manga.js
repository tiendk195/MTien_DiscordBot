...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("name")
        .setDescription(`${language.__n(`manga.manga_name`)}`)
        .setRequired(true)
    ),
    ...
    try {
      ...
      if (!MangaData) {
        return interaction.reply(
          `${language.__n(`global.no_results`)} **${MangaName}**`
        );
      }

      const description = MangaData.description
        ? MangaData.description.slice(0, 500) + "..."
        : "Không có thông tin.";
